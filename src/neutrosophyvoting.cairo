...
trait VoteTrait<T> {
    ...
    fn get_vote_status(self: @T) -> (u8, u8, u8, u8, u8, u8);
...
mod Vote {
    ...
    const YES: u8 = 1_u8;
    const NO: u8 = 0_u8;
    const UNDECIDED: u8 = 2_u8; // isso é novo
    ...
    struct Storage {
        yes_votes: u8,
        no_votes: u8,
        undecided_votes: u8, // isso é novo
    ...
    fn constructor(
        ...
        self.undecided_votes.write(0_u8); // isso é novo
    }
    ...
    impl VoteImpl of super::VoteTrait<ContractState> {
        ...
        fn get_vote_status(self: @ContractState) -> (u8, u8, u8, u8, u8, u8) {
            let (n_yes, n_no, n_und) = self._get_voting_result(); // isso é novo
            let (yes_percentage, no_percentage, undecided_percentage) = self
                ._get_voting_result_in_percentage(); // isso é novo
            (
                n_yes, n_no, n_und, yes_percentage, no_percentage, undecided_percentage
            ) // isso é novo
        }
        ...
        fn vote(ref self: ContractState, vote: u8) {
            assert!(
                vote == NO || vote == YES || vote == UNDECIDED, "VOTE_0_OR_1_OR_2"
            ); // aqui deu pau
            ...
            if (vote == UNDECIDED) {
                self.undecided_votes.write(self.undecided_votes.read() + 1_u8); // isso é novo
            }
    ...
    impl VoteResultFunctionsImpl of VoteResultFunctionsTrait {
        ...
        fn _get_voting_result(self: @ContractState) -> (u8, u8, u8) {
            let n_yes: u8 = self.yes_votes.read();
            let n_no: u8 = self.no_votes.read();
            let n_und: u8 = self.undecided_votes.read(); // isso é novo

            (n_yes, n_no, n_und)
        }

        // @dev Internal function to calculate the voting results in percentage
        fn _get_voting_result_in_percentage(self: @ContractState) -> (u8, u8, u8) {
            let n_yes: u8 = self.yes_votes.read();
            let n_no: u8 = self.no_votes.read();
            let n_und: u8 = self.undecided_votes.read(); // isso é novo

            let total_votes: u8 = n_yes + n_no + n_und;
            ...
            let undecided_percentage: u8 = (n_und * 100_u8) / (total_votes); // isso é novo

            (yes_percentage, no_percentage, undecided_percentage)
        }
    }
}
